...
void Modifier_joueur::Db_Afficher_Liste_Matchs()
{
     ...
     model->setTable("JOUEUR"); //Définition de la base sur laquelle opère le model
...
void Modifier_joueur::onTableClicked(const QModelIndex &index)
{
    ...
    if (index.isValid()) {
     ...
     QSqlQuery qry;
     ui->id_sup_8->clear();
     qry.prepare("select * from JOUEUR WHERE NOM='"+val+"' or PRENOM='"+val+"' or IMAGE='"+val+"'");
     if(qry.exec())
     {
         while(qry.next())
         {
...
void Modifier_joueur::submit()
{
   ...
   image=filename;
       ...
       qry.prepare("update JOUEUR set IMAGE='"+image+"' where ID_J='"+save_id+"' ");
