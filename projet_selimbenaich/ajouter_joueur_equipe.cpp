    ...
    ui(new Ui::ajouter_joueur_equipe)
{
    ...
    QSqlQueryModel * Modal1=new  QSqlQueryModel();
     QSqlQuery qry,qry1;
     qry.prepare("SELECT NOM FROM JOUEUR WHERE ID_J NOT IN (SELECT ID_JE FROM CONTENIR) ");
     qry1.prepare("SELECT NOM_EQUIPE FROM EQUIPE");
     qry.exec();
     qry1.exec();
     Modal->setQuery(qry);
...
void ajouter_joueur_equipe::on_Ajouter_joueur_equipe_clicked()
{
    ...
    QSqlQuery qry,qry1,qry2,qry3,qry_bonus;
        ...
        if(nom_equipe.isEmpty()){
          QMessageBox::critical(this,tr("Appliquer"),tr("Veuillez Selectionner un joueur."));
        }
       else if(id_joueur.isEmpty()){
          QMessageBox::critical(this,tr("Applique"),tr("Veuillez Selectionner une équipe."));
        }
        else {
    qry_bonus.prepare(" select j.* from joueur j, contenir c where j.id_j=c.id_je and c.nom_eq='"+nom_equipe+"' ");
    qry.prepare("insert into CONTENIR (ID_JE,NOM_EQ) select j.ID_J,e.NOM_EQUIPE from JOUEUR j,EQUIPE e where j.ID_J='"+id_joueur+"' and e.NOM_EQUIPE='"+nom_equipe+"' ");
    qry1.prepare("update EQUIPE set TOTAL_JOUEUR=TOTAL_JOUEUR+1 where NOM_EQUIPE='"+nom_equipe+"'");
    qry2.prepare("SELECT NOM FROM JOUEUR WHERE ID_J NOT IN (SELECT ID_JE FROM CONTENIR) ");
    qry3.prepare("SELECT NOM_EQUIPE FROM EQUIPE");
