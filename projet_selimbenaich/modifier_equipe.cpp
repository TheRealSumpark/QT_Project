...
void modifier_equipe::on_afficher_modif_activated(const QModelIndex &index)
{
   ...
   qry1.prepare("select C.NOM_EQ from Contenir_temp CO,CONTENIR C WHERE C.id_je=CO.id_je");
    if(qry.exec())
    {
        while(qry.next())
        {
...
qry3.prepare("select id_je from Contenir_temp WHERE id_je='"+save_id_joueur+"'");
if(qry3.exec())
{
while(qry3.next())
       {
...
//qry5.prepare("select C.NOM_EQ from CONTENIR_TEMP CO,CONTENIR C WHERE CO.ID_JE=C.id_je");
