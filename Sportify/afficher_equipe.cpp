    ...
    ui(new Ui::afficher_equipe)
{
    ...
    QSqlQueryModel * Modal1=new  QSqlQueryModel();
     QSqlQuery qry;
     qry.prepare("SELECT NOM_EQUIPE FROM \"Sportify\".\"EQUIPE\" order by score ASC ");
     qry.exec();
     Modal1->setQuery(qry);
...
void afficher_equipe::on_comboBox_equipe_currentIndexChanged(const QString &arg1)
{
    ...
    qry1.prepare("SELECT NOM FROM \"Sportify\".\"JOUEUR\" WHERE ID_J IN (SELECT ID_JE FROM \"Sportify\".\"CONTENIR\" WHERE NOM_EQ='"+val+"' ) ");
    if(qry.exec())
    {
        while(qry.next())
        {
