    ...
    ui(new Ui::supprimer_equipe)
{
    ...
    QSqlQueryModel * Modal1=new  QSqlQueryModel();
     QSqlQuery qry;
     qry.prepare("SELECT NOM_EQUIPE FROM \"Sportify\".\"EQUIPE\"");
     qry.exec();
     Modal1->setQuery(qry);
...
void supprimer_equipe::on_comboBox_equipe_currentIndexChanged(const QString &arg1)
{
    ...
    qry1.prepare("SELECT * FROM \"Sportify\".\"JOUEUR\" WHERE ID_J IN (SELECT ID_JE FROM \"Sportify\".\"CONTENIR\" WHERE NOM_EQ='"+val+"' ) ");
    if(qry.exec())
    {
        while(qry.next())
        {
...
void supprimer_equipe::on_supprimer_equipe_2_clicked()
{
    ...
    qry.prepare("delete from \"Sportify\".\"EQUIPE\" WHERE NOM_EQUIPE='"+save_nom_equipe+"'");
    qry1.prepare("delete from \"Sportify\".\"CONTENIR\"WHERE NOM_EQ='"+save_nom_equipe+"'");
