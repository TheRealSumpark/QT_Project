    ...
    ui(new Ui::Afficher_joueur)
{
    ...
    QPixmap pic("C:/Users/toshiba/Desktop/Sportify/Integration/images_projet/projet77c.png");
     ...
     QSqlTableModel * Modal1=new QSqlTableModel;
      ...
      qry.exec();
      Modal->setQuery(qry);
      Modal1->setTable("\"Sportify\".\"JOUEUR\"");
      Modal1->select();
          ...
          for (unsigned int i=0;i<vectStat.size();i++)
                    {
                        ...
                        QDateTime time=QDateTime::fromString(timeString,"yyyy-MM-ddThh:mm:ss.zzz");
...
{
    QSqlDatabase db1;
    QModelIndex id_index = ui->tableView->model()->index(index.row(),0, QModelIndex());
