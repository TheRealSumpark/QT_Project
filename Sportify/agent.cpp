...
void addValues(QString cin,QString nom,QString prenom,QString mdp)
{
    ...
    QSqlQuery qry;
    qry.prepare("INSERT INTO \"Sportify\".\"AGENTS\" ("
                "CIN,"
                "NOM,"
                "PRENOM,"
                "MDP)"
...
{
    QSqlQuery qry;
    qry.prepare("update  \"Sportify\".\"AGENTS\" set NOM='"+nom+"',PRENOM='"+prenom+"',MDP='"+mdp+"' where CIN ='"+cin+"' ");
    qry.addBindValue(nom);
    qry.addBindValue(prenom);
    qry.addBindValue(mdp);
    if(!qry.exec())
...
{
    QSqlQuery qry;
    qry.prepare("DELETE FROM \"Sportify\".\"AGENTS\" where CIN='"+cin+"'");
    if(!qry.exec())
    {
    }
...
void Agent::on_pushButton_clicked()
{
    ...
    qry.prepare("SELECT * FROM \"Sportify\".\"AGENTS\"");
    qry.exec();
    modal->setQuery(qry);
...
void Agent::on_pushButton_rechercher_clicked()
{
    ...
    QSqlQuery qry;
    QSqlQueryModel *modal=new QSqlQueryModel();
    qry.prepare("SELECT * FROM \"Sportify\".\"AGENTS\" WHERE NOM LIKE '"+a+"' OR MDP LIKE '"+a+"' OR PRENOM LIKE '"+a+"' OR CIN LIKE '"+a+"'");
    qry.exec();
    modal->setQuery(qry);
    ui->tableagent->setModel(modal);
}
...
void Agent::on_pushButton_4_clicked()
{
  ...
  modal = new QSqlTableModel(this);
      modal->setTable("\"Sportify\".\"AGENTS\"");
    modal ->select();
...
void Agent::on_ajouter_Ag_3_clicked()
{
...

    {
        ...
        QSqlQuery qry;
        qry.prepare("INSERT INTO \"Sportify\".\"PARTENAIRE\" ("
                    "CIN,"
                    "NOM,"
                    "ADRESSE,"
                    "DONS)"
...
void Agent::on_pushButton_7_clicked()
{
    QSqlQuery qry;
    QSqlQueryModel *modal=new QSqlQueryModel();
    qry.prepare("SELECT * from \"Sportify\".\"PARTENAIRE\"");
    qry.exec();
    modal->setQuery(qry);
...
void Agent::on_pushButton_8_clicked()
{
  ...
  modal = new QSqlTableModel(this);
      modal->setTable("\"Sportify\".\"PARTENAIRE\"");
    modal ->select();
...
void Agent::on_supprimeragent_3_clicked()
{
    ...
    QString a;
    a=ui->lineEdit_cin_3->text();
    qry.prepare("DELETE FROM \"Sportify\".\"PARTENAIRE\" where CIN='"+a+"'");
    if(!qry.exec())
    {
    }
}
...
void Agent::on_pushButton_rechercher_3_clicked()
{
    ...
    QSqlQuery qry;
    QSqlQueryModel *modal=new QSqlQueryModel();
    qry.prepare("SELECT * FROM \"Sportify\".\"PARTENAIRE\" WHERE NOM LIKE '"+a+"' OR DONS LIKE '"+a+"' OR ADRESSE LIKE '"+a+"' OR CIN LIKE '"+a+"'");
    qry.exec();
    modal->setQuery(qry);
    ui->tableagent_3->setModel(modal);
}
